...
df_nan_ana = fit.groupby('lvl3permid')[features].apply(lambda x: x.isna().mean()).T
df_nan_ana_dro = fit.groupby('lvl3permid')[dropped_features].apply(lambda x: x.isna().mean()).T
df_nan_ana_ret = fit.groupby('lvl3permid')[retained_features].apply(lambda x: x.isna().mean()).T


print(df_nan_ana_ret)
print(df_nan_ana_dro)
...
for i, region in enumerate(regions):
    ...
    region_data = df_nan_ana[region].sort_values(ascending=True)
